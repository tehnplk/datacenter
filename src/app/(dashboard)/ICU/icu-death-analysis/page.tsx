...
export default async function Page({
  ...
  const years = await dbQuery<{ y: number }>(
    `SELECT DISTINCT y FROM public.transform_sync_icu_ward_death ORDER BY y DESC`,
  ).then((r) => r.map((x) => x.y));
  ...
  const [top10, rows, meta] = await Promise.all([
    dbQuery<Top10Row>(
      `SELECT d.pdx, ic.name AS pdx_name, sum(d.death_count)::int AS total_death,
       ...
       FROM public.transform_sync_icu_ward_death d
    ...
    dbQuery<{ last_update: string | null }>(
      `SELECT max(d_update)::text AS last_update
       FROM public.transform_sync_icu_ward_death WHERE y = $1`,
      [selectedYear],
    ).then((r) => r[0]),
  ]);
  ...
  return (
    <MetricPage
      title="การตายใน ICU / semi ICU (Top 10)"
      description="10 อันดับโรคที่มีผู้เสียชีวิตสูงสุดใน ICU / semi ICU"
      showTopCards={false}
      contentWidth="full"
    >
      ...
      {years.length === 0 && (
        <div className="mt-6 rounded-xl bg-zinc-50 px-6 py-10 text-center text-sm text-zinc-500 ring-1 ring-zinc-200/70 dark:bg-zinc-900 dark:text-zinc-400 dark:ring-white/10">
          ยังไม่มีข้อมูลในตาราง transform_sync_icu_ward_death
        </div>
      )}
          ...
          {selectedTab === "overview" && (
            <div className="overflow-hidden rounded-b-xl rounded-tr-xl ring-1 ring-zinc-200/70 dark:ring-white/10">
              ...
              <div className="overflow-auto bg-white dark:bg-zinc-950">
                {top10.length === 0 ? (
                  <div className="px-4 py-10 text-center text-sm text-zinc-500 dark:text-zinc-400">ไม่พบข้อมูลปี {selectedYear}</div>
                ) : (
              ...
              <div className="border-t border-zinc-200 bg-white px-3 py-1.5 text-right text-[11px] text-zinc-400 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-500">
                ข้อมูลจากตาราง: <span className="font-mono">transform_sync_icu_ward_death</span>
              </div>
            </div>
          )}
